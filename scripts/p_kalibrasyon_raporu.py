...
def kapali_dongu(X, Y, *, n_grid: int = N_IZGARA,
    ...
    """θ-grubu başına bırak-birini kapalı döngü vakaları."""
    space = DART_UZAYI_S3
    X = np.asarray(X, float)
    Y = np.asarray(Y, float)
    grup = _gruplar(X)
    eksen = np.linspace(0.0, 1.0, n_grid)
    izg = np.meshgrid(*([eksen] * space.ndim), indexing="ij")
    A_izg = design_matrix(np.column_stack([a.ravel() for a in izg]))
    U = space.to_unit(X)
    vakalar = []
    for g in np.unique(grup):
        egit = grup != g
        C = np.empty((A_izg.shape[1], Y.shape[1]))
        E = np.empty((int(egit.sum()), Y.shape[1]))
        for i in range(Y.shape[1]):
            C[:, i] = fit_surrogate(space, X[egit], Y[egit, i]).coef
            E[:, i] = loo_artiklari(space, X[egit], Y[egit, i])
        S = kuculmus_kov(E)
        tahmin = A_izg @ C
        for r in np.flatnonzero(~egit):
            vaka = {"grup": int(g), "gercek_u": U[r].tolist(), "carpan": {}}
            for c in carpanlar:
                post = grid_posterior_kovaryans(space, tahmin, Y[r], c * S, n_grid)
                vaka["carpan"][float(c)] = {
                    "hdi68": post.hdi_u.tolist(),
                    "hdi95": [_aralik(post, j, 0.025, 0.975)
...
def rapor(kayitlar: list[dict], s_n: dict | None, *, n_grid: int = N_IZGARA) -> dict:
    ...
    if not secilen:
        ...
        return out
    X = np.array([k["theta"] for k in kayitlar], float)
    Y = np.array([[k[g] for g in secilen] for k in kayitlar], float)
    iyi = np.all(np.isfinite(Y), axis=1)
    X, Y = X[iyi], Y[iyi]
    vakalar = kapali_dongu(X, Y, n_grid=n_grid)
    ...
    out.update(n_kosu=int(len(X)), n_theta=int(len(np.unique(_gruplar(X)))),
               eksen=eksenler, gurultu_tepkisi=tepki,
