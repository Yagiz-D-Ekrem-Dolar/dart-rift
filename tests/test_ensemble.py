...
# ---------------------------------------------------------------------------
# FAZ 5 surucusu — IKI ASAMALI modele bagli mi, devam ediyor mu
# ---------------------------------------------------------------------------

def test_faz5_surucusu_IKI_ASAMALI_modele_bagli():
    """FAZ 5 tek aşamalı ileri modeli **kullanmamalı**.

    `faz46_sentetik_kurtarma.py` kendi tek aşamalı `ileri_kosu`'sunu
    kullanıyor (`λ = 2`, `A1 = 0,215`) ve KAYIT-045 ölçtü ki o **başka
    bir problemi** çözüyor: `n_ejekta = 803`, yani merminin **tamamı**
    sekiyor. Çözülmüşte `28`. Rejim değişikliği.
    """
    from pathlib import Path
    kaynak = (Path(__file__).resolve().parents[1] / "scripts"
              / "faz5_ensemble.py").read_text(encoding="utf-8")
    assert "ileri_kosu_ikiasama" in kaynak
    assert "from dartrift.inference.forward import ileri_kosu\n" not in kaynak
    # Dusen nokta SESSIZCE tamamlanmis sayilmamali
    assert "raise RuntimeError" in kaynak


def test_faz5_kesintiden_sonra_KALDIGI_YERDEN(tmp_path):
    """Kesinti bir olasılık değil **zorunluluk**: `~50 GPU-günü` /
    `3 gün` duvar sınırı.

    Sürücü tamamlanmış noktaları atlamazsa her kesinti baştan başlamak
    demektir ve ensemble hiç bitmez.
    """
    import numpy as np
    from dartrift.inference.ensemble import ensemble_kos

    X = np.linspace(0.0, 1.0, 12)[:, None]
    yol = tmp_path / "iz.jsonl"
    cagri = {"n": 0}

    def ileri_kesintili(th):
        cagri["n"] += 1
        if cagri["n"] > 5:
            raise KeyboardInterrupt("duvar suresi")
        return np.array([float(th[0]) * 2.0])

    try:
        ensemble_kos(X, ileri_kesintili, yol, root_seed=7)
    except KeyboardInterrupt:
        pass
    ilk = sum(1 for _ in yol.read_text(encoding="utf-8").splitlines() if _.strip())
    assert ilk >= 5, f"kesintiye kadar {ilk} nokta yazilmis"

    # IKINCI kosu: yazilanlari ATLAMALI
    cagri["n"] = 0
    d = ensemble_kos(X, lambda th: np.array([float(th[0]) * 2.0]),
                     yol, root_seed=7)
    assert d.atlanan == ilk, f"atlanan {d.atlanan}, beklenen {ilk}"
    # `tamamlanan` ATLANANLARI DA KAPSIYOR (ilk yazdigimda iki kez
    # saymistim): toplam = tamamlanan + dusen.
    assert d.tamamlanan + d.dusen == len(X), (d.tamamlanan, d.dusen)
    assert d.tamamlanan >= d.atlanan
